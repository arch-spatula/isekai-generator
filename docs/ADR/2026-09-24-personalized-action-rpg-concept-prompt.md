...
# 31. Decision — Tauri Desktop Shell과 출생 프로필 입력 UI

## Context

생성 작업은 AI 모델, 로컬 파일, 장시간 실행되는 작업 큐와 연결될 수 있다. 단순한 브라우저
페이지로만 제공하면 로컬 작업 상태, 티저·오픈 베타 산출물, 시스템 알림 및 백그라운드
작업을 하나의 사용자 경험으로 묶기 어렵다.

또한 게임 생성의 시작점에는 사용자의 출생 프로필을 입력하고, 입력 과정 자체를 명식과
연결된 시각적 경험으로 보여주는 화면이 필요하다.

## Decision

초기 데스크톱 애플리케이션 셸은 **Tauri**로 확정한다.

Tauri는 다음 책임을 갖는다.

- 로컬 웹 UI를 데스크톱 창으로 제공한다.
- 출생 프로필 입력 UI를 표시한다.
- Rust 백엔드와 프론트엔드 사이에서 생성 작업 상태를 전달한다.
- 티저, 클로즈 베타, 완성 게임의 준비 상태를 표시한다.
- 생성 완료 시 버튼을 활성화하고 필요하면 시스템 알림을 표시한다.
- 작업 상태와 생성 산출물의 로컬 경로를 관리한다.

Tauri를 게임 본편의 렌더링 엔진으로 사용한다는 의미는 아니다. 생성된 2D 게임은 기존
결정에 따라 LÖVE2D를 우선 사용하며, Tauri는 입력·생성·대기·산출물 확인을 담당하는
애플리케이션 셸로 둔다.
...
태어난 시간은 필수로 취급한다. 출생 시간 미상 모드는 지원하지 않는다.

태어난 도시를 선택하면 도시명과 함께 계산에 필요한 위치·시간대 정보를 내부적으로
확정한다. 입력값은 사주팔자, 자미두수, 네이탈 차트 계산 및 게임 Seed 생성 단계로
전달한다.

성별 필드는 점술 계산 엔진이 요구하는 의미와 사용자의 정체성을 혼동하지 않도록 별도
라벨과 설명을 제공한다. 실제 필수 여부와 허용 값은 계산 엔진 계약에서 구체화한다.

## 명식 탐색과 톱니바퀴 상호작용

사용자는 방향키를 이용해 입력값 또는 계산된 명식의 항목을 이동·변경할 수 있다. 방향키로
명식이 변경될 때마다 화면 배경에 배치된 명식 표현 톱니바퀴가 회전한다.

상호작용 원칙:

- 방향키 입력과 명식 변경 결과가 일대일로 연결되어야 한다.
- 명식이 실제로 변경된 경우에만 톱니바퀴를 회전시킨다.
- 키를 누르는 동안 회전 애니메이션을 과도하게 누적하지 않는다.
- 값이 유효하지 않아 변경되지 않은 입력에는 성공 상태의 회전을 표시하지 않는다.
- 톱니바퀴 애니메이션은 명식의 계산 결과를 장식적으로 표현하며, 회전 각도 자체가
  점술적 의미를 갖는 것으로 해석하지 않는다.
- 키보드를 사용하지 않는 사용자도 동일한 변경과 회전 피드백을 얻을 수 있도록 버튼,
  포커스 이동 또는 접근성 입력을 제공한다.
...
Tauri UI는 애플리케이션이 실행 중인 동안 생성 백엔드가 보내는 상태 이벤트를 수신한다.
티저나 오픈 베타가 준비되면 페이지를 새로고침하거나 앱을 재시작하지 않고 관련 버튼을
활성화한다.
...
이벤트는 실시간 화면 갱신에 사용하고, 작업 상태와 산출물 목록은 파일 또는 SQLite에
저장한다. 앱이 다시 실행되거나 일시적으로 이벤트를 놓친 경우에도 저장된 상태를 조회해
동일한 화면을 복원할 수 있어야 한다.

Tauri 창을 닫을 때 생성 작업까지 반드시 종료할지는 별도 운영 정책으로 남긴다. 작업이
앱 종료 후에도 계속되어야 하는 경우 생성 워커를 Tauri UI 프로세스와 분리한다.

## Consequences

### Positive

- 웹 UI의 빠른 개발성과 데스크톱 앱의 로컬 파일·알림 접근을 함께 얻는다.
- 출생 프로필 입력, 명식 시각화, 생성 진행, 티저 및 베타 제공을 하나의 창에서 연결할 수 있다.
- 이벤트와 영속 상태를 결합해 앱 재시작 없이 갱신하면서도 작업 상태를 복구할 수 있다.
- 완성된 LÖVE2D 게임과 생성 관리 UI의 책임을 분리할 수 있다.

### Negative

- Tauri의 Rust 백엔드와 프론트엔드 양쪽을 유지해야 한다.
- 운영체제별 알림·트레이·창 닫기 동작을 검증해야 한다.
- Tauri 앱이 완전히 종료된 뒤에도 작업하려면 별도 워커나 원격 실행 환경이 필요하다.
- 출생정보와 생성 산출물의 로컬 저장·삭제 정책을 함께 설계해야 한다.

## Related Documents

- [출생 프로필 입력 UI 명세](../spec/birth-profile-input-ui.md)
- [개인화 Action RPG 생성 Master Prompt](../prompts/personalized-action-rpg-concept.md)

이 결정으로 Tauri 사용을 확정하지만, Tauri 플러그인 종류, 백그라운드 워커의 실행 방식,
로컬 저장소 형식 및 톱니바퀴의 시각 디자인은 후속 ADR 또는 상세 명세에서 결정한다.
