...
Lua 코드를 작성하거나 수정할 때 LuaLS가 인식하는 타입 주석으로 타입을 명시하세요. Neovim에서 hover, 자동 완성, 타입 검사를 활용할 수 있어야 합니다.

- 함수의 매개변수는 `---@param`, 반환값은 `---@return`으로 명시하세요.
- 변수와 테이블은 `---@type`으로 타입을 명시하고, 구조를 정의할 때는 `---@class`와 `---@field`를 사용하세요.
- LÖVE API는 `.luarc.json`에 연결된 타입 정의를 활용하세요.
- 변경 범위에 포함되지 않는 기존 코드까지 타입 주석을 일괄 추가하지 마세요.

```lua
---@param damage number
---@param defense number
---@return number
local function calculateDamage(damage, defense)
